...
class MalariaPreprocessor:
    ...
    def preprocess(self):
        """
        Preprocesses the data by adding 'month', 'week', and 'target' columns.
        """
        ...
        # Calculate historical median for (month, week)
        self.median_values = self.data.groupby(['month', 'week'])[self.target_column].median()

        # Classify as higher or lower than median
        def classify_cases(row):
            median_cases = self.median_values.loc[row['month'], row['week']]
            return 1 if row[self.target_column] > median_cases else 0

        self.data['target'] = self.data.apply(classify_cases, axis=1)
        print("Data preprocessing completed. Added 'target' column.")
    ...
    def get_classification_summary(self):
        """
        Returns a summary of classifications and median thresholds.
        """
        ...
        # Prepare the median thresholds for each (month, week)
        median_thresholds = self.median_values.reset_index().rename(columns={self.target_column: 'Median'})

        # Create the summary
        summary = {
            "Class 1 Count (Higher)": class_1_count,
            "Class 0 Count (Lower)": class_0_count,
            "Median Thresholds": median_thresholds  # Include the DataFrame here
        }
        return summary
    def save_with_classification(self):
        """
        Saves the updated dataset with the 'Median_classification' column to the original file.
        """
        self.data.to_csv(self.data_path, index=False)
        print(f"Updated data with 'Median_classification' saved to: {self.data_path}")